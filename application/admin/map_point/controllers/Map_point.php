...
class Map_point extends MY_Controller
{
	private $data = array();
	...
	public function  __construct()
	{
		...
		$this->load->model(array('Map_point_model','icon/Icon_model','client_site/Client_site_model','cabang/Cabang_model'));
		...
		$this->data['html_js'] = '
			...
			<script type="text/javascript">
				...
				function newData()
				{
					window.open("'.base_url().'index.php/map_point/create","_self");
				}

				function editData()
				{
					var row = $("#dg").datagrid("getSelected");
					window.open("'.base_url().'index.php/map_point/edit/"+row.ID_INFO,"_self");
				}

				function deleteData()
				{
					var row = $("#dg").datagrid("getSelected");
					window.open("'.base_url().'index.php/map_point/delete/"+row.ID_INFO,"_self");
				}

				function viewData()
				{
					var row = $("#dg").datagrid("getSelected");
					window.open("'.base_url().'index.php/map_point/view/"+row.ID_INFO,"_self");
				}	
			</script>
			...
			<script>
				...
				{
					window.open("'.base_url().'index.php/map_point/add","_self");
				}

				function editMapPoint()
				{
					var row = $("#dg").datagrid("getSelected");
					window.open("'.base_url().'index.php/map_point/edit/"+row.ID,"_self");
				}
				...
				function destroyMapPoint()
				{
					if(confirm("Are you sure ?")) {
						var row = $("#dg").datagrid("getSelected");
						window.open("'.base_url().'index.php/map_point/delete/"+row.ID,"_self");
					}
				}
				...
				function showModalFileManager(action=0,latitude=0,longitude=0) {
					...
					if(action == 1) {
						$("#map-modal").find("iframe").attr("src","'.base_url().'index.php/map_point/view_list");
					}

					if(action == 2) {
						$("#map-modal").find("iframe").attr("src","'.base_url().'index.php/map_point/add?lat="+latitude+"&lon="+longitude);
					}
				}
				...
				app.CustomToolbarControl = function(opt_options) {
				  ...
				  selectList.onchange = function(e){
				  	    vectorSource.clear();
				        $.get("'.base_url().'index.php/map_point/get_point/"+this.value,function(json) {
				      	if(json.total > 0) {
	...
	public function index()
	{
		...
		$this->load->view('map_point_view',$this->data);
	...
	public function view_list()
	{
		...
		$this->load->view('admin/header',$this->data);
		$this->load->view('map_point_list_view',$this->data);
		$this->load->view('admin/footer',$this->data);
	}
	...
	{
		$query = $this->Map_point_model->get_all_items();
	...
	public function get_point($type="")
	{
		...
		$query = $this->Map_point_model->get_by_type($type);
		$json_object = new stdClass();
		$json_object->total = @$query->num_rows();		
		$json_object->rows  = @$query->result();
		header('Content-Type: application/json');
	...
	public function add()
	{
		...
		$this->load->view('admin/header',$this->data);
		$this->load->view('map_point_add_view',$this->data);
		$this->load->view('admin/footer',$this->data);
	}
	...
	public function edit($id=0)
	{
		...
		$this->data['title'] = "Map Point Management";
		$this->data['id']    = $id;
		$this->data['item']  = $this->Map_point_model->get_item_by_id($id);
		$this->load->view('admin/header',$this->data);
		$this->load->view('map_point_edit_view',$this->data);
		$this->load->view('admin/footer',$this->data);
	}
	...
	{
		$this->Map_point_model->delete_by_id($id);
